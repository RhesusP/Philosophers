...
/*   Updated: 2023/04/27 11:56:58 by cbernot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "./../includes/philo.h"

void	check_philo_duration(t_params *params)
{
	int					i;
	...
	while (i < params->nb_philos && !is_philo_dead(params))
	{
		...
		{
			print_action(&(params->philo_tab[i]), "died");
...
void	*death_checker_routine(void *arg)
{
	...
	while (!did_all_philos_eat(params))
	{
		...
		while (params->max_meal != -1 && i < params->nb_philos && \
			nb_meal >= params->max_meal)
			i++;
